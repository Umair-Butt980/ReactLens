...
import { AsyncVisual } from '@/components/visualizations';
import { asyncCode, asyncSteps, asyncKeyTakeaways } from '@/lib/data/async-steps';
import type { PlaybackSpeed } from '@/lib/types';

export default function AsyncPromisesPage() {
  const [currentStep, setCurrentStep] = useState(1);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState<PlaybackSpeed>(1);

  const totalSteps = asyncSteps.length;
  const currentStepData = asyncSteps[currentStep - 1];
  ...
  return (
    <div className="bg-background flex min-h-screen flex-col">
      ...
      <ConceptHeader
        ...
        nextTopic={{ title: 'Prototypes', href: '/javascript/prototypes' }}
      />
        ...
        <SplitView
          animationPanel={
            <AnimationPanelWrapper title="Visualization">
              <AsyncVisual state={currentStepData.animationState} />
            </AnimationPanelWrapper>
          }
          codePanel={
            <CodePanel
      ...
      <ExplanationPanel
        ...
        controls={
          <div className="flex w-full flex-col items-center gap-6">
            ...
            {currentStep === totalSteps && (
              <KeyTakeaways takeaways={asyncKeyTakeaways} className="w-full max-w-xl" />
            )}
          </div>
        }
      />
