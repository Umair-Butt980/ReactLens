...
Fiber enables React to schedule work based on priority, keeping your UI responsive.`,
    ...
    state: {
      ...
      timeline: [],
      lanes: [
        { lane: 'SyncLane', label: 'Sync (Urgent)', color: '#EF4444', pendingWork: 0, isActive: false },
        { lane: 'InputContinuousLane', label: 'Input', color: '#F97316', pendingWork: 0, isActive: false },
        { lane: 'DefaultLane', label: 'Default', color: '#EAB308', pendingWork: 0, isActive: false },
        { lane: 'TransitionLane', label: 'Transition', color: '#22C55E', pendingWork: 0, isActive: false },
...
When you call setState, React clones current fiber into WIP, applies changes there, then swaps when done.`,
    ...
    state: {
      ...
      workUnits: [],
      timeline: [
        { id: 'ev-1', type: 'schedule', description: 'setState(count + 1) scheduled', timestamp: Date.now(), isActive: true },
      ],
      lanes: [
...
After each fiber, React checks: "Do I need to yield?" If yes, it saves workInProgress and returns. Next frame, it resumes from exactly where it left off.`,
    ...
    state: {
      ...
      workUnits: [
        { id: 'wu-1', fiberId: 'wip-root', fiberName: 'Root', phase: 'complete', isProcessed: true, timestamp: Date.now() - 2 },
        { id: 'wu-2', fiberId: 'wip-app', fiberName: 'App', phase: 'complete', isProcessed: true, timestamp: Date.now() - 1 },
        { id: 'wu-3', fiberId: 'wip-div', fiberName: 'div', phase: 'begin', isProcessed: false, timestamp: Date.now() },
      ],
      timeline: [
        { id: 'ev-1', type: 'begin', fiberId: 'wip-root', fiberName: 'Root', description: 'beginWork(Root)', timestamp: Date.now() - 3, isActive: false },
        { id: 'ev-2', type: 'begin', fiberId: 'wip-app', fiberName: 'App', description: 'beginWork(App)', timestamp: Date.now() - 2, isActive: false },
        { id: 'ev-3', type: 'begin', fiberId: 'wip-div', fiberName: 'div', description: 'beginWork(div)', timestamp: Date.now(), isActive: true },
      ],
      lanes: [
        { lane: 'SyncLane', label: 'Sync', color: '#EF4444', pendingWork: 1, isActive: true },
        { lane: 'InputContinuousLane', label: 'Input', color: '#F97316', pendingWork: 0, isActive: false },
...
Without time slicing, processing 1000 fibers might take 100ms with NO interruption. With slicing, it takes the same total time, but stays responsive!`,
    ...
    state: {
      ...
      workUnits: [],
      timeline: [
        { id: 'ev-1', type: 'begin', description: 'Started processing...', timestamp: Date.now() - 10, isActive: false },
        { id: 'ev-2', type: 'yield', description: 'shouldYield() → true, pausing', timestamp: Date.now() - 5, isActive: true },
        { id: 'ev-3', type: 'resume', description: 'Browser handled events, resuming...', timestamp: Date.now(), isActive: true },
      ],
      lanes: [
        { lane: 'SyncLane', label: 'Sync', color: '#EF4444', pendingWork: 0, isActive: false },
        { lane: 'InputContinuousLane', label: 'Input', color: '#F97316', pendingWork: 1, isActive: false },
        { lane: 'DefaultLane', label: 'Default', color: '#EAB308', pendingWork: 0, isActive: false },
        { lane: 'TransitionLane', label: 'Transition', color: '#22C55E', pendingWork: 1, isActive: true },
        { lane: 'IdleLane', label: 'Idle', color: '#6B7280', pendingWork: 0, isActive: false },
      ],
      output: [
...
React processes SyncLane first, INTERRUPTING the data update if needed. User sees responsive typing, data loads in background.`,
    ...
    state: {
      ...
      workUnits: [],
      timeline: [
        { id: 'ev-1', type: 'schedule', description: 'Click: setCount() → SyncLane', timestamp: Date.now() - 2, isActive: true },
        { id: 'ev-2', type: 'schedule', description: 'startTransition: setItems() → TransitionLane', timestamp: Date.now() - 1, isActive: false },
      ],
      lanes: [
        { lane: 'SyncLane', label: 'Sync (Click)', color: '#EF4444', pendingWork: 1, isActive: true },
        { lane: 'InputContinuousLane', label: 'Input', color: '#F97316', pendingWork: 0, isActive: false },
        { lane: 'DefaultLane', label: 'Default', color: '#EAB308', pendingWork: 0, isActive: false },
        { lane: 'TransitionLane', label: 'Transition (Data)', color: '#22C55E', pendingWork: 1, isActive: false },
        { lane: 'IdleLane', label: 'Idle', color: '#6B7280', pendingWork: 0, isActive: false },
      ],
      output: [
...
Combined with Suspense, this enables truly fluid UIs that never feel "stuck".`,
    ...
    state: {
      ...
      workUnits: [],
      timeline: [
        { id: 'ev-1', type: 'schedule', description: 'startTransition(() => setItems(...))', timestamp: Date.now() - 5, isActive: false },
        { id: 'ev-2', type: 'begin', description: 'Processing 1000 items...', timestamp: Date.now(), isActive: true },
      ],
      lanes: [
        { lane: 'SyncLane', label: 'Sync', color: '#EF4444', pendingWork: 0, isActive: false },
        { lane: 'InputContinuousLane', label: 'Input', color: '#F97316', pendingWork: 0, isActive: false },
...
The work loop did the heavy lifting (diffing, creating fibers). Commit just applies the results.`,
    ...
    state: {
      ...
      workUnits: [],
      timeline: [
        { id: 'ev-1', type: 'complete', description: 'Render phase complete', timestamp: Date.now() - 3, isActive: false },
        { id: 'ev-2', type: 'commit', description: 'Commit: Before mutation', timestamp: Date.now() - 2, isActive: false },
        { id: 'ev-3', type: 'commit', description: 'Commit: Mutation (DOM updates)', timestamp: Date.now(), isActive: true },
      ],
      lanes: [
        { lane: 'SyncLane', label: 'Sync', color: '#EF4444', pendingWork: 0, isActive: false },
        { lane: 'InputContinuousLane', label: 'Input', color: '#F97316', pendingWork: 0, isActive: false },
...
This architecture enables React's concurrent features: Suspense, transitions, and responsive UIs even during heavy computation!`,
    ...
    state: {
      ...
      output: [
        ...
        { id: 'msg-3', type: 'info', message: 'Fiber enables: Concurrent features (Suspense, transitions)' },
      ],
      activePanel: 'trees',
      showAlternate: false,
      showPointers: true,
